...
void keyboardMesh( unsigned char key, int x, int y )
{
    ...
    switch ( key ) {
	  ...
	  break;
      case 'n':			// MST
	  MSTMerge( mesh, skeleton, joint, pattern, paper, bound );
	  arrangeMultiple	( pattern, paper, bound );
	  // Drawing the MST
	  drawing_mode = 3;
	  break;
	  ...
	  partial = whole;
	  assignWeights		( mesh, partial, weight_type );
	  spanningTrees		( partial, mesh, skeleton, pattern, paper, bound );
	  countNCuts		( mesh );
	  labelBoundary		( mesh, attr );
	  drawing_mode = 3;
	  break;
...
//------------------------------------------------------------------------------
    ...
    glutAddMenuEntry	( "MST",		MINIMUM_SPANNINGTREE);
